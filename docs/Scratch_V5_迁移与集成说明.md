...
当前配置中的权重路径是原开发机绝对路径。迁移后必须修改，否则会从旧目录加载或直接报找不到文件。
...
当前 `infer_scratch_v5.py` 不会自动执行上述相对路径解析。若原样复制现有脚本，应把 JSON 中三处 `weights` 改成主项目中的绝对路径；更推荐在迁移时实现配置相对路径解析。
