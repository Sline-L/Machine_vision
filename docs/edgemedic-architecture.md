...
| A3 experimental validation | future work (pilot `python -m edgemedic.a3 --phase pilot`) |
| A4 | out of scope |
...
| C Dataset replay | locator + Scratch V5 + CUDA/Jetson telemetry on NX | frames from disk, not `/dev/video0` | Healthy 4-combo window recorded. A3 GPU-pressure pilot harness ready, not yet run. `gpu_mem_mb` still null. |
