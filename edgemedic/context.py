...
def component_health(snapshot):
    out = {}
    nodes = ("camera", "locator", "scratch_v5", "serial", "missing_hole_v1")
    for node in nodes:
        block = dict(snapshot.get(node) or {})
        if node == "missing_hole_v1" and not block:
            block = dict(((snapshot.get("specialists") or {}).get("missing_hole_v1")) or {})
        if node == "scratch_v5" and not block:
            block = dict(((snapshot.get("specialists") or {}).get("scratch_v5")) or {})
        out[node] = {
