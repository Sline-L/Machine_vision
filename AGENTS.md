...
The headless Web application is developed on `main-web`. The `srtp` branch
retains the Qt application; `archive-old` is the pre-refactor tree.
...
- Keep the `main-web` runtime headless; do not reintroduce Qt into `gp/`.
